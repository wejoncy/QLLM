...
template <>
int Dequantize4Bits<c10::Half>(c10::Half *output, const uint8_t *quant_data,
                               const c10::Half *scales_data,
                               const uint8_t *zero_points, int k, int n,
                               int block_size, cudaStream_t stream) {
  return Dequantize4Bits<half>(reinterpret_cast<half *>(output), quant_data,
                               reinterpret_cast<const half *>(scales_data),
                               zero_points, k, n, block_size, stream);
}

template int Dequantize4Bits<float>(
    float* output,
    const uint8_t* quant_data,
...
template int Dequantize4Bits<half>(half *output, const uint8_t *quant_data,
